...
def main():
    ...
    for label in set(constants.cargo_labels):
        assert constants.cargo_labels.count(label) == 1, (
            "Cargo %s is defined more than once"
        ) % label
    ...
    dist_file_header = codecs.open(
        os.path.join(currentdir, "src", "dist_file_header.txt"), "r", "utf8"
    ).read()
    for filename in [
        "__init__.py",
        "constants.py",
        ...
        dist_file.close()
    for dir_name in ["piece_cargos", "intermodal_containers", "vehicles_cargos"]:
